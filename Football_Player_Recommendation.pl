...
main:-
    writeln("Please select the height range of the player:"),
    writeln("    1 170 - 175 cm"),
...
physicality(PlayerHeight):-
    writeln("Please select the Physicality of the player:"),
    writeln("    1 21 - 30"),
    writeln("    2 31 - 40"),
    writeln("    3 41 - 50"),
    writeln("    4 51 - 60"),
...
price(PlayerHeight, PlayerPhysicality, PlayerRate):-
    ...
    writeln("    1 10,000    - 100,000"),
    writeln("    2 100,001   - 500,000"),
    writeln("    3 500,001   - 1000000"),
    writeln("    4 1000001   - 10,000000"),
    writeln("    5 10,000001 - 30,000000"),
    writeln("    6 30,000001 - 50,000000"),
    writeln("    7 50,000001 - 60,000000"),
    writeln("    8 60,000001 - 100,000000"),
...
position(PlayerHeight, PlayerPhysicality, PlayerRate, PlayerPrice):-
    ...
    writeln("    4  Goalkeeper  |   Player has quick reflexes and blocking skills"),
...
forward(PlayerHeight, PlayerPhysicality, PlayerRate, PlayerPrice):-
    writeln("What kind of Forward player are you looking for?"),nl,
    writeln("    1  Winger   |   Plays on the flanks"),
    writeln("    2  Striker  |   Primary goal scorer"),
...
striker(PlayerHeight, PlayerPhysicality, PlayerRate, PlayerPrice):-
    Position = 'Striker',
    ...
    writeln("Will you prefer a player with Best Right foot? (yes/no)"),
    ...
    find_results_for_striker(_, _, Position, _, PlayerHeight, PlayerPhysicality, PlayerRate, PlayerPrice, _, Shooting, _, Goals, _, _, _, _, Foot, Results),

    (Results = [] -> write("Sorry, based on the given criteria no suitable player found! Please change your criteria"); writeln("System recommends the following Players:"),nl, print_list(Results)), end.
...
find_results_for_striker(_, _, Position, _, PlayerHeight, PlayerPhysicality, PlayerRate, PlayerPrice, _, Shooting, _, Goals, _, _, _, _, Foot, Results) :-
    findall((Name, Team, Price), (
        ...
        nonvar(Name),
        within_range(Shoot, Shooting),
        within_range(Goal, Goals),
        within_range(Rate, PlayerRate),
        within_range(Price, PlayerPrice)
    ), Results).


midfielder(PlayerHeight, PlayerPhysicality, PlayerRate, PlayerPrice):-
    Position = 'Midfielder',

    writeln("Select the type of Midfielder? Enter number from the menu:"),
    writeln("    1 Defensive Midfielder (CDM) - Protects the defense."),
    writeln("    2 Central Midfielder (CM) - Plays in the middle"),
    writeln("    3 Attacking Midfielder (CAM) - Supports the attack"),
    ...
    find_results_for_midfielder(_, _, Position, SubPosition, PlayerHeight, PlayerPhysicality, PlayerRate, PlayerPrice, _, _, _, _, _, Passing, Dribbling, _, _, Results),
...
find_results_for_midfielder(_, _, Position, SubPosition, PlayerHeight, PlayerPhysicality, PlayerRate, PlayerPrice, _, _, _, _, _, Passing, Dribbling, _, _, Results) :-
    findall((Name, Team, Price), (
        ...
        nonvar(Name),
        within_range(Dribble, Dribbling),
        within_range(Rate, PlayerRate),
        within_range(Price, PlayerPrice),
        within_range(Pass, Passing)
    ), Results).



winger(PlayerHeight, PlayerPhysicality, PlayerRate, PlayerPrice):-
    Position = 'Winger',

    writeln("Select the type of Winger? Enter number from the menu:"),
    writeln("    1 Right Winger (RW)"),
    ...
    (   (PlayerPace < 1; PlayerPace > 8) -> writeln("Invalid Input! You must write number between 1 and 8"), winger(PlayerHeight, PlayerPhysicality, PlayerRate, PlayerPrice); writeln("")),
     ...
     find_results_for_winger(_, _, Position, SubPosition, PlayerHeight, PlayerPhysicality, PlayerRate, PlayerPrice, _, _, _, _, _, _, _, Pace, _, Results),
...
find_results_for_winger(_, _, Position, SubPosition, PlayerHeight, PlayerPhysicality, PlayerRate, PlayerPrice, _, _, _, _, _, _, _, Pace, _, Results) :-
    findall((Name, Team, Price),(
        ...
        within_range(Rate, PlayerRate),
        within_range(Price, PlayerPrice),
        within_range(Pc, Pace)
    ), Results).


defender(PlayerHeight, PlayerPhysicality, PlayerRate, PlayerPrice):-
    Position = 'Defender',

    writeln("Select the type of Defender? Enter number from the menu:"),
    writeln("    1 Right Back (RB) - Defends on the right side"),
    ...
    writeln("Select the age range for the player:"),
    writeln("    1 20 - 25 yrs"),
    writeln("    2 26 - 30 yrs"),
    writeln("    3 31 - 35 yrs"),
    writeln("    4 36 - 40 yrs"),
    ...
    find_results_for_defender(Age, _, Position, SubPosition, PlayerHeight, PlayerPhysicality, PlayerRate, PlayerPrice, _, _, _, _, Defending, _, _, _, _, Results),
...
find_results_for_defender(Age, _, Position, SubPosition, PlayerHeight, PlayerPhysicality, PlayerRate, PlayerPrice, _, _, _, _, Defending, _, _, _, _, Results) :-
    findall((Name, Team, Price), (
        ...
        within_range(Rate, PlayerRate),
        within_range(Price, PlayerPrice),
        within_range(Defend, Defending)
    ), Results).


goalkeeper(PlayerHeight, PlayerPhysicality, PlayerRate, PlayerPrice):-
    Position = 'Goalkeeper',
    ...
    (   (PlayerShooting < 1; PlayerShooting > 8) -> writeln("Invalid Input! You must write number between 1 and 8"), goalkeeper(PlayerHeight, PlayerPhysicality, PlayerRate, PlayerPrice); writeln("")),
     ...
     find_results_for_goalkeeper(_, _, Position, _, PlayerHeight, PlayerPhysicality, PlayerRate, PlayerPrice, _, Shooting, Footworks, _, _, _, _, _, _, Results),
     (Results = [] -> write("Sorry, based on the given criteria no suitable player found! Please change your criteria"); writeln("System recommends the following Players:"),nl, print_list(Results)), end.
...
print_list([]).
print_list([(Name, Team, Price)|T]) :-
    format("Name: ~w,  Team: ~w,  Price: ~w EUR~n", [Name, Team, Price]),
    print_list(T).
...
within_range(Value, [Min, Max]) :-
    ...
    writeln(""),nl,
    writeln("---------------------------------------------------------------------------------------------"),
    writeln("                               Thank you for using our system!                               "),
    writeln("---------------------------------------------------------------------------------------------").
